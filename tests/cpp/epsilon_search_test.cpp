...
int main() {
  ...
  int ef_construction =
      200;  // Controls index search speed/build speed tradeoff

  int num_queries = 100;
  float epsilon2 = 1.0;  // Squared distance to query
  int max_num_candidates =
      max_elements;  // Upper bound on the number of returned elements in the
                     // epsilon region
  int min_num_candidates =
      2000;  // Minimum number of candidates to search in the epsilon region
             // this parameter is similar to ef

  // Initing index
  hnswlib::L2Space space(dim);
  hnswlib::BruteforceSearch<dist_t>* alg_brute =
      new hnswlib::BruteforceSearch<dist_t>(&space, max_elements);
  hnswlib::HierarchicalNSW<dist_t>* alg_hnsw =
      new hnswlib::HierarchicalNSW<dist_t>(&space, max_elements, M,
                                           ef_construction);
  ...
  for (int i = 0; i < num_queries; i++) {
    ...
    hnswlib::EpsilonSearchStopCondition<dist_t> stop_condition(
        epsilon2, min_num_candidates, max_num_candidates);
  ...
  for (size_t i = 0; i < max_elements; i++) {
    hnswlib::EpsilonSearchStopCondition<dist_t> stop_condition(
        epsilon2_small, min_candidates_small, max_num_candidates);
    std::vector<std::pair<float, hnswlib::labeltype>> result =
        alg_hnsw->searchStopConditionClosest(alg_hnsw->getDataByInternalId(i),
                                             stop_condition);
