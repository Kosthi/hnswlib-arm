...
int main() {
  ...
  int max_elements =
      1000;    // Maximum number of elements, should be known beforehand
  ...
  int ef_collection = 15;  // Number of candidate documents during the search
                           // Controlls the recall: higher ef leads to better
  ...
  std::uniform_int_distribution<docidtype> distrib_docid(min_doc_id,
                                                         max_doc_id);
  ...
  for (int i = 0; i < max_elements; i++) {
    ...
    std::vector<std::pair<float, hnswlib::labeltype>> result =
        alg_hnsw->searchStopConditionClosest(data + i * data_point_size,
                                             stop_condition);
