...
int main() {
  ...
  hnswlib::HierarchicalNSW<float>* alg_hnsw =
      new hnswlib::HierarchicalNSW<float>(&space, 2 * max_elements);
  ...
  while (true) {
    ...
    std::uniform_int_distribution<> distrib_int(start_label,
                                                start_label + num_labels - 1);
  ...
  // create threads that will do markDeleted and unmarkDeleted of random
  // elements each thread works with specific range of labels
  ...
  num_threads = 20;
  std::uniform_int_distribution<> distrib_int_add(max_elements,
                                                  2 * max_elements - 1);
  for (size_t thread_id = 0; thread_id < num_threads; thread_id++) {
    threads.push_back(std::thread([&] {
      std::vector<float> data(d);
