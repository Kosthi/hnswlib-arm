...
void get_gt(float* mass, float* massQ, size_t vecsize, size_t qsize,
            L2Space& l2space, size_t vecdim,
  ...
  (vector<std::priority_queue<std::pair<float, labeltype>>>(qsize))
      .swap(answers);
  // answers.swap(vector<std::priority_queue< std::pair< float, labeltype
  // >>>(qsize));
  for (int i = 0; i < qsize; i++) {
    std::priority_queue<std::pair<float, labeltype>> gt =
        bs.searchKnn(massQ + vecdim * i, 10);
    answers[i] = gt;
  }
}

void get_gt(unsigned int* massQA, float* massQ, float* mass, size_t vecsize,
            size_t qsize, L2Space& l2space, size_t vecdim,
            vector<std::priority_queue<std::pair<float, labeltype>>>& answers,
            size_t k) {
  // answers.swap(vector<std::priority_queue< std::pair< float, labeltype
  // >>>(qsize));
  ...
  for (int i = 0; i < qsize; i++) {
    for (int j = 0; j < k; j++) {
      float other =
          fstdistfunc_(massQ + i * vecdim, mass + massQA[100 * i + j] * vecdim,
...
float test_approx(
    float* massQ, size_t vecsize, size_t qsize,
  ...
  for (int i = 0; i < qsize; i++) {
    std::priority_queue<std::pair<float, labeltype>> result =
        appr_alg.searchKnn(massQ + vecdim * i, 10);
    ...
    while (result.size()) {
      if (g.find(result.top().second) != g.end()) correct++;
      result.pop();
    }
  }
...
void test_vs_recall(
    ...
    vector<std::priority_queue<std::pair<float, labeltype>>>& answers,
    size_t k) {
  // vector<size_t> efs = { 1,2,3,4,6,8,12,16,24,32,64,128,256,320 };//  = ; {
  // 23 };
  vector<size_t> efs;
  for (int i = 10; i < 30; i++) {
    efs.push_back(i);
...
// void get_knn_quality(unsigned int *massA,size_t vecsize, size_t maxn,
// HierarchicalNSW<float> &appr_alg) {
//     size_t total = 0;
//     size_t correct = 0;
//     for (int i = 0; i < vecsize; i++) {
//         int *data = (int *)(appr_alg.linkList0_ + i *
//         appr_alg.size_links_per_element0_);
//         //cout << "numconn:" << *data<<"\n";
...
// #define LOAD_I
  ...
  // HierarchicalNSW<float> appr_alg(&l2space,
  // "D:/stuff/hnsw_lib/nmslib/similarity_search/release/temp",true);
  // HierarchicalNSW<float> appr_alg(&l2space,
  // "/mnt/d/stuff/hnsw_lib/nmslib/similarity_search/release/temp", true);
...
#pragma omp parallel for
  ...
  //        = appr_alg.searchKnnInternal(appr_alg.getDataByInternalId(i), 17);
  ...
  //
  //        for (tableint l : g) {
  //            float other = fstdistfunc_(appr_alg.getDataByInternalId(l),
  //            appr_alg.getDataByInternalId(i), l2space.get_dist_func_param());
  //            rez.emplace(other, l);
  //        }
  //        while (rez.size() > 32)
  //            rez.pop();
