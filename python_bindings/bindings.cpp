...
inline std::vector<size_t> get_input_ids_and_check_shapes(
    const py::object& ids_, size_t feature_rows) {
 ...
 public:
  ...
  py::object getData(py::object ids_ = py::none(),
    ...
    if (return_type == "numpy") {
      return py::array_t < data_t,
             py::array::c_style | py::array::forcecast > (py::cast(data));
    }
  }
  ...
  py::dict getAnnData() const { /* WARNING: Index::getAnnData is not thread-safe
    ...
    for (size_t i = 0; i < appr_alg->cur_element_count; i++) {
      ...
      if (linkListSize) link_npy_size += linkListSize;
    }
  ...
  py::dict getIndexParams() const { /* WARNING: Index::getAnnData is not
    ...
    if (index_inited == false) return py::dict(**params, "ef"_a = default_ef);
  ...
  void setAnnData(const py::dict d) { /* WARNING: Index::setAnnData is not
    ...
    assert_true(
        appr_alg->ef_construction_ == d["ef_construction"].cast<size_t>(),
